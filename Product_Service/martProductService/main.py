...
async def read_image(image_id: int, session: Annotated[Session, Depends(db_session)]):
...
async def create_product(product: ProductCreate, session: Annotated[Session, Depends(db_session)], user: Annotated[User, Depends(check_admin)]):
...
async def read_products(session: Annotated[Session, Depends(db_session)], skip: int = 0, limit: int = 10):
    products = session.exec(select(Product).offset(skip).limit(limit)).all()
    return products

@app.get("/search", response_model=List[Product], tags=["Product"])
async def get_product_by_name(product_name: str, session: Annotated[Session, Depends(db_session)]):
...
async def read_product_by_id(product_id: int, session: Annotated[Session, Depends(db_session)]):
...
async def delete_product(product_id: int, session: Annotated[Session, Depends(db_session)], user: Annotated[User, Depends(check_admin)]):
    product = session.get(Product, product_id)
    if not product:
        raise HTTPException(status_code=404, detail="Product not found")
